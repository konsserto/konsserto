 ...
 */
var Bundle, CONFIG, ExceptionHandler, MIDDLEWARES, MiddlewareDefinition, MiddlewareInstance, ROUTING, RouteDefinition, RouteInstance, Router, SECURITY, SOCKET, SocketDefinition, SocketInstance, bodyparser, cc, cookieParser, csrf, express, logger, session, wait, xssFilter,
...
Router = (function() {
  ...
  Router.prototype.initHttpLogger = function() {
    var accessLogStream;
    if (CONFIG.loggerHTTP) {
      accessLogStream = use('fs').createWriteStream('./app/logs/http.log', {
        flags: 'a'
      });
      this.app.use(logger('combined', {
        stream: accessLogStream
      }));
      return console.log('[Logger] HTTP Logger OK');
    }
  };

  Router.prototype.initSecurity = function() {
    if (SECURITY.xssProtection === void 0 || SECURITY.xssProtection === true) {
      console.log('[Security] Protection against XSS attacks explicitly OK');
      return this.app.use(xssFilter());
    }
  };
  ...
  Router.prototype.cookie_session_csrf = function() {
    if (CONFIG.secret && CONFIG.secret !== '') {
      ...
      return console.log(cc.red('[WARNING] CSRF protection disabled, you didn\'t specified a secret key in the /app/config/config file !'));
    }
  };
  ...
  Router.prototype.setHeaders = function() {
    ...
    if (SECURITY.headers_allowed != null) {
      console.log('[Security] Headers allowed :');
      _ref = SECURITY.headers_allowed;
    ...
    return this.app.all('*', (function(_this) {
      return function(req, res, next) {
        var _j, _len1, _ref1;
        if (SECURITY.headers_allowed != null) {
          _ref1 = SECURITY.headers_allowed;
