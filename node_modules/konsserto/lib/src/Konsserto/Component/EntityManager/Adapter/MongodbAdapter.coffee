...
class MongodbAdapter
  ...
  getById: (collectionName, id)->
    deferred = Q.defer()
    MongodbAdapter.CONNECTION.collection(collectionName).findOne({_id: new ObjectID(id)}, (err, result)->
      if err
        deferred.reject(new Error(err))
      else
        deferred.resolve(result)
  ...
  add: (collectionName, obj)->
    deferred = Q.defer()
    MongodbAdapter.CONNECTION.collection(collectionName).insert(@makeObjectPersistant(obj), (err, result)->
      if err
        deferred.reject(new Error(err))
      else
        if result.length is 1
          obj.attributes._id.value = result[0]._id
  ...
  makeObjectPersistant: (object)->
    objectPersistant = {}
    array_types = ['string', 'int', 'long', 'time', 'date']
    for k,v of object.attributes
      if !v.dbIgnore
        if v.type in array_types
          ...
          if Array.isArray(v.value)
            ...
            for value in v.value
              objectPersistant[k].push {$ref: v.type,$id: value.attributes._id.value,}
          else
            objectPersistant[k] = {$ref: v.type,$id: v.value.attributes._id.value,}
  ...
  modifyById: (collectionName, id, newObject)->
    deferred = Q.defer()
    MongodbAdapter.CONNECTION.collection(collectionName).update({_id:new ObjectID(id)}, @makeObjectPersistant(newObject), (err, result)->
      if err
        deferred.reject(new Error(err))
      else
        if result.length is 1
