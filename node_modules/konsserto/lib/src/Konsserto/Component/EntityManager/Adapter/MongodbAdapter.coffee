...
class MongodbAdapter
  ...
  add: (collectionName, obj)->
    object = @makeObjectPersistant(obj)
    deferred = Q.defer()
    MongodbAdapter.CONNECTION.collection(collectionName).insert(object, (err, result)->
      if err
        deferred.reject(new Error(error))
      else
        ...
        if v.type in array_types
          ...
          if Array.isArray(v.value)
            ...
            for value in v.value
              object[k].push {$id: value.attributes._id.value, $ref: v.type}
          else
            object[k] = {$id: v.value.attributes._id.value, $ref: v.type}
