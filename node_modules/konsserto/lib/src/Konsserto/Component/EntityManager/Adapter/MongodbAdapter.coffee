      ...
      if k != 'repository'
        if !v.dbIgnore
          if v.type in array_types
            if k isnt 'id' # si on ne traite pas l'id
              objectPersistant[k] = v.value
          else
            if typeof(v) == 'object' # si lelement est un complexe object
              if Array.isArray(v.value)
                objectPersistant[k] = []
                for value in v.value
                  objectPersistant[k].push {$ref: v.type, $id: value.id.value, }
              else
                objectPersistant[k] = {$ref: v.type, $id: v.id.value, }
