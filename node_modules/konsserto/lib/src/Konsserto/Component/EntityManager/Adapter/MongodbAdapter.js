...
MongodbAdapter = (function() {
  ...
  MongodbAdapter.prototype.add = function(collectionName, object, callback) {
    return MongodbAdapter.CONNECTION.collection(collectionName).insert(object, function(err, result) {
      if (err) {
        throw new Error(err);
      } else {
        if (result.length === 1) {
