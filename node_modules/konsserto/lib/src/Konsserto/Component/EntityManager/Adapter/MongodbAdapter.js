...
MongodbAdapter = (function() {
  ...
  MongodbAdapter.prototype.add = function(collectionName, obj) {
    var deferred, object;
    object = this.makeObjectPersistant(obj);
    deferred = Q.defer();
    MongodbAdapter.CONNECTION.collection(collectionName).insert(object, function(err, result) {
      if (err) {
        return deferred.reject(new Error(error));
      } else {
        if (result.length === 1) {
  ...
  MongodbAdapter.prototype.makeObjectPersistant = function(object) {
    var array_types, k, v, value, _i, _len, _ref, _ref1, _ref2;
    ...
    for (k in _ref) {
      ...
      if (v.dbIgnore) {
        ...
        if (_ref1 = v.type, __indexOf.call(array_types, _ref1) >= 0) {
          object[k] = v.value;
        } else {
          if (Array.isArray(v.value)) {
            ...
            for (_i = 0, _len = _ref2.length; _i < _len; _i++) {
              value = _ref2[_i];
              object[k].push({
                $id: value.attributes._id.value,
                $ref: v.type
              });
            }
          } else {
            object[k] = {
              $id: v.value.attributes._id.value,
              $ref: v.type
            };
          }
        }
