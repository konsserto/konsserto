  ...
  flush: ()->
    for object in @objectToPersist
      if object.state is 'managed'
        repository = @getRepository(object.getRepository())
        if object.attributes._id.value isnt null
          repository.modifyById(object.attributes._id.value, object)
        else
          repository.add(object).then((newValue)->
          )
      else
