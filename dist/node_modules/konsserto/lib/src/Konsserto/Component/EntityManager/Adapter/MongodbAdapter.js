var DBCONFIG, MongoClient, MongodbAdapter, ObjectID, ObjectMaker, Q, cc;
...
MongodbAdapter = (function() {
  ...
  function MongodbAdapter() {
    this.objectMaker = new ObjectMaker(this);
  }
  ...
  MongodbAdapter.prototype.add = function(collectionName, obj) {
    var deferred;
    deferred = Q.defer();
    MongodbAdapter.CONNECTION.collection(collectionName).insert(obj, function(err, result) {
      if (err) {
        return deferred.reject(new Error(err));
      } else {
        if (result.length === 1) {
